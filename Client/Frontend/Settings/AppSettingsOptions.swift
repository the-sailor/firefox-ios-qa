...
// MARK: App Setting Generator
func generateAppSettings(settingsVC: SettingsTableViewController, settingsDelegate: SettingsDelegate?) -> [SettingSection] {
    var settings = [SettingSection]()
    let profile = settingsVC.profile
    ...
    if AppConstants.BuildChannel != .Aurora {
        accountDebugSettings = [
            // Debug settings:
            RequirePasswordDebugSetting(settings: settingsVC),
            RequireUpgradeDebugSetting(settings: settingsVC),
            ForgetSyncAuthStateDebugSetting(settings: settingsVC),
        ]
    } else {
        accountDebugSettings = []
    }
    ...
    var generalSettings = [
        SearchSetting(settings: settingsVC),
    ...
    if locale.localeIdentifier != "zh_CN" {
        ...
        accountChinaSyncSetting = [
            ...
            ChinaSyncServiceSetting(settings: settingsVC)
        ]
    }
    ...
    settings += [
        SettingSection(title: nil, children: [
            ...
            ConnectSetting(settings: settingsVC),
            // With a Firefox Account:
            AccountStatusSetting(settings: settingsVC),
            SyncNowSetting(settings: settingsVC)
    ...
    if AppConstants.MOZ_LOGIN_MANAGER {
        privacySettings.append(LoginsSetting(settings: settingsVC, delegate: settingsDelegate))
    }

    if AppConstants.MOZ_AUTHENTICATION_MANAGER {
        privacySettings.append(TouchIDPasscodeSetting(settings: settingsVC))
    }

    privacySettings.append(ClearPrivateDataSetting(settings: settingsVC))
    ...
    settings += [
        ...
        SettingSection(title: NSAttributedString(string: NSLocalizedString("Support", comment: "Support section title")), children: [
            ShowIntroductionSetting(settings: settingsVC),
            SendFeedbackSetting(),
            OpenSupportPageSetting(delegate: settingsDelegate),
        ]),
        SettingSection(title: NSAttributedString(string: NSLocalizedString("About", comment: "About settings section title")), children: [
            VersionSetting(settings: settingsVC),
            LicenseAndAcknowledgementsSetting(),
            YourRightsSetting(),
            DisconnectSetting(settings: settingsVC),
            ExportBrowserDataSetting(settings: settingsVC),
            DeleteExportedDataSetting(settings: settingsVC),
        ])
    ]
